...
export function useAuth() {
  ...
  const sessionInvalidatedThisSession = sessionStorage.getItem(
    'session_invalidated'
  )
  ...
  const userQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.status === 'error') {
        console.warn(
          '[useAuth] loadUser failed, checking localStorage fallback'
        )
  ...
  const login = (newUserId: string, userData?: User, sessionToken?: string) => {
    ...
    console.log(
      '[useAuth] login complete, userId set to:',
      newUserId,
      'localStorage:',
