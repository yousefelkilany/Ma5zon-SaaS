...
export function WarehousesSubTable({
  ...
  useEffect(() => {
    if (!productsCache.has(warehouseId) && !localProductsCache.has(warehouseId)) {
      ...
      typedCommands
        .productsGetByWarehouseWithStock(warehouseId, productPagination.pageSize, 0)
        .then(result => {
          if (result.status === 'ok') {
            ...
            setErrorProducts(prev => new Map(prev).set(warehouseId, result.error))
          }
        })
        .catch(e => {
  ...
  }, [warehouseId, productsCache, localProductsCache, typedCommands, productPagination.pageSize])
  ...
  if (stockLevels.length === 0 && products.length === 0) {
    return (
...
export function WarehousesSubTable({
  ...
  useEffect(() => {
    if (
      ...
      typedCommands
        .productsGetByWarehouseWithStock(warehouseId)
        .then(result => {
          if (result.status === 'ok') {
            setLocalProductsCache(prev =>
              new Map(prev).set(warehouseId, result.data)
  ...
  }, [warehouseId, productsCache, localProductsCache, typedCommands])
